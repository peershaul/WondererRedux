...
void wondererTextureLoaderTerminate() {
   ...
   wondererDynamicArrayFree(&texture_pool);
}

static Texture create_texture(GLenum *params) {
   GLuint id;
   glGenTextures(1, &id);
   ...
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, params[0]);
   glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, params[1]);
...
Texture *wondererTextureCreateWithParameters(GLenum *parameters){
   ...
   if(found != -1){
      arr[found] = create_texture(parameters);
      return &arr[found];
   }
   ...
   arr[texture_pool.last] = create_texture(parameters);
...
Texture *wondererTextureCreate() {
   ...
   if(found != -1){
      arr[found] = create_texture(texture_params);
      return &arr[found];
   }
   ...
   arr[texture_pool.last] = create_texture(texture_params);
   return &arr[texture_pool.last++];
}
...
void wondererTextureSetImageParams(Texture *tex, uint16_t width, uint16_t height, 
                                  GLenum color_mode){

   Texture *curr_bound = wondererTextureGetBound(curr_slot);
   wondererTextureBind(curr_slot, tex);

   glTexImage2D(GL_TEXTURE_2D, 0, color_mode, width, height, 0, color_mode, 
